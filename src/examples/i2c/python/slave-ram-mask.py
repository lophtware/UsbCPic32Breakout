...
# RAM Address mask.
#
# When invoked with an integer argument, sets the device's Protected RAM
# Address mask.
#
...
def set_protected_ram_address_mask(dev, mask):
	report_id = 0x10
...
def get_protected_ram_address_mask(dev):
	report_id = 0x10
...
if __name__ == "__main__":
	...
	try:
		...
		if len(sys.argv) == 2:
			...
			ack = set_protected_ram_address_mask(dev, mask)
			if ack[0] != 0:
				print("Error 0x{:02x} whilst trying to set address mask to 0x{:04x}".format(ack[0], mask))
			else:
				print("Protected RAM Address Mask: 0x{0:04x}".format(mask))

		else:
			mask = get_protected_ram_address_mask(dev)
			print("Protected RAM Address Mask: 0x{0:04x}".format(mask))

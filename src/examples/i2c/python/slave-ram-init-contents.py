...
#
# Purpose:
# Allows getting and setting the Slave's initial RAM contents.
#
...
def send_read_raminit(dev):
	report_id = 0x16
	return dev.get_input_report(report_id, 1037)

def send_write_raminit(dev, contents, offset):
	report_id = 0x16
	unlock_key = [0x00] * 8
...
def do_read(dev, offset, count, filename):
	print("Reading RAM Initialisation [0x{:03x}, 0x{:03x}]...".format(offset, offset + count - 1))
	report = send_read_raminit(dev)
...
def do_write(dev, offset, count, filename):
	...
	try:
		print("Writing RAM Initialisation [0x{:03x}, 0x{:03x}]...".format(offset, offset + count - 1))
		send_write_raminit(dev, list(contents), offset)
		ack = dev.read(16)
		if len(ack) > 0:
...
if __name__ == "__main__":
	...
	try:
		...
		if len(sys.argv) != 5:
			print("Usage: slave-ram-init-contents.py <read|write> <offset> <count> <filename>")

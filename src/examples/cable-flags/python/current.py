...
#
# Purpose:
# Lights A3 if the port is capable of delivering >= 100mA, and C9 if it is
# capable of delivering >= 1500mA.
#
...
def configure_a3_as_capable_of(dev, milliamps):
	milliamps = int((milliamps + 1) / 2)
	configure_pin_as_flag_output(dev, 0x03, [
		0x00, 0x40,
		0x00, 0x00,
		(milliamps >> 0) & 0xff, (milliamps >> 8) & 0x03])

def configure_c9_as_capable_of(dev, milliamps):
	milliamps = int((milliamps + 1) / 2)
	configure_pin_as_flag_output(dev, 0x29, [
		0x00, 0x40,
		0x00, 0x00,
		(milliamps >> 0) & 0xff, (milliamps >> 8) & 0x03])
...
if __name__ == "__main__":
	...
	try:
		...
		configure_a3_as_capable_of(dev, 100)
		configure_c9_as_capable_of(dev, 1500)
